...
가장 먼저 build.gradle에 다음과 같은 의존성을 추가 해 주도록 하자. 
...
| 패키지 경로                             | 파일이름      |
|------------------------------------|-----------|
| com.samsungsds.springai.controller | CSVReader |
...
## VectorDB 설정
백터DB는 Pinecone을 사용 할 예정이다. 다만 최근에 Pinecone에서 무료로 사용할 수 있는 인덱스를 생성하는 방법이 바뀌어서 아직 Spring에 반영되어 있지 않은 상태이다. 
따라서 본 실습에서는 웹 페이지에서 직접 인덱스를 생성 하도록 하겠다. 

사전 환경설정 시 가입했던 계정으로 Pinecone에 접속 해 주면 다음과 같은 화면을 볼 수 있다. 
여기서 Create Index를 클릭 해 주도록 하자.
![4.png](img%2F04%2F4.png)

인덱스 이름과 Dimension을 다음과 같이 설정 해 주도록 하자. 
![7.png](img%2F04%2F7.png)

| 이름       | Dimensions |
|----------|------------|
| bbc-news | 1536       |

이후 설정들은 아래 이미지와 같이 해 준다. 
무료 버전에서는 AWS의 Virginia 리전에서만 사용 가능하다.
![8.png](img%2F04%2F8.png)

설정이 완료된 후 Create index를 클릭 해 주면 인덱스 생성이 완료 된다. 
![9.png](img%2F04%2F9.png)

생성이 완료되면 위와같은 URL을 볼 수 있는데, 이 URL의 내용을 application.properties에 설정 해 주어야 한다. 

URL은 다음과 같이 구성이 되어 있다. 각각 항목을 application.properties에 설정 해 주도록 하자. 

https://bbc-news-59n65pl.svc.aped-4627-b74a.pinecone.io<br>
구조 : {index-name}-{projectId}.svc.{environment}.pinecone.io

이를 application.properties에 설정 하면 다음과 같다. 
...
`index-name`은 우리가 입력 한 bbc-news로 넣어주면 되고 `projectId`와 `environment`는 URL에서 확인한 내용을 넣어주면 된다.
...
dependencies {
    ...
    implementation platform("org.springframework.ai:spring-ai-bom:1.0.0-SNAPSHOT") 
    implementation 'org.springframework.ai:spring-ai-openai-spring-boot-starter'    
    implementation 'org.springframework.ai:spring-ai-pinecone-store-spring-boot-starter' // Pinecone Store
...
spring.ai.openai.embedding.api-key=spring.ai.openai.embedding.api-key=sk-proj-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
spring.ai.openai.embedding.options.model=text-embedding-3-small
```
<br><br>
본 실습에서는 가장 작은 모델인 text-embedding-3-small을 사용 하도록 하겠다.
현재 OpenAI에서는 다음과 같은 모델을 제공하고 있다.
![10.png](img%2F04%2F10.png)
...
![11.png](img%2F04%2F11.png)

`/store-data`를 클릭 해서 실행 해 보도록 하자. 
![12.png](img%2F04%2F12.png)
